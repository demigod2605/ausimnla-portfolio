...
const logo = (scale) => `
  <g transform="translate(${600 - 16 * scale}, ${170 - 16 * scale}) scale(${scale})">
    ...
    <path d="M5 11 V5 H11" stroke="${BLUE}" stroke-width="1.5" fill="none"/>
    <path d="M21 5 H27 V11" stroke="${BLUE}" stroke-width="1.5" fill="none"/>
    <path d="M27 21 V27 H21" stroke="${BLUE}" stroke-width="1.5" fill="none"/>
    <path d="M11 27 H5 V21" stroke="${BLUE}" stroke-width="1.5" fill="none"/>
...
const grid = () => {
  ...
  for (let x = 0; x <= 1200; x += 42) {
    lines += `<line x1="${x}" y1="0" x2="${x}" y2="630" stroke="${BLUE}" opacity="0.04"/>`;
  }
  for (let y = 0; y <= 630; y += 42) {
    lines += `<line x1="0" y1="${y}" x2="1200" y2="${y}" stroke="${BLUE}" opacity="0.04"/>`;
  }
  return lines;
};
...
<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630">
  <rect width="1200" height="630" fill="#04050F"/>
  ${grid()}
  ...
  <rect x="520" y="90" width="160" height="160" rx="24" fill="#0A0D1E" stroke="${BLUE}" stroke-width="6"/>
  ${logo(4)}
  ...
  <text x="600" y="400" text-anchor="middle" font-size="34" letter-spacing="12" fill="${BLUE}" font-family="DejaVu Sans Mono, monospace">FRONTEND / UI DEVELOPER</text>
</svg>`;
